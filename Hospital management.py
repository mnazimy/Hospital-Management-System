    ...
    for i in Doctor_DataBase:
        for j in Doctor_DataBase[i] :
            if str(patient_ID) == str(j[0]) :
                Appointment_index = Doctor_DataBase[i].index(j)
...
while tries_flag != "Close the program":
        ...
        Patients_DataBase = Read_Hospital_Excel_Sheet.Read_Patient_DataBase()
        Doctor_DataBase = Read_Hospital_Excel_Sheet.Read_Doctors_Database()
        print("-----------------------------------------")
        ...
        if Admin_user_mode == "1":  # Admin mode
            ...
            while True:
                if Password == "1234":
                        ...
                        while True:
                            ...
                            if Admin_choice == "1":  # Admin mode ; Doctors Management ; Enter new doctor data
                                try:  # To avoid non integer input
                                    ...
                                    while Doctor_ID in Doctor_DataBase:  # if Admin entered used ID
                                    ...
                                    Doctor_DataBase[Doctor_ID]=[[Department,Name,Address]]
                                ...
                                try:  # To avoid non integer input
                                    Doctor_ID = int(input("Enter doctor ID : "))
                                    while Doctor_ID not in Doctor_DataBase:
                                        Doctor_ID = int(input("Incorrect ID, Please Enter doctor ID : "))
                                    print("Doctor name    : ", Doctor_DataBase[Doctor_ID][0][1])
                                    print("Doctor address : ", Doctor_DataBase[Doctor_ID][0][2])
                                    print("Doctor is in " + Doctor_DataBase[Doctor_ID][0][0]+" department")
                                except:
                                    print("Doctor ID should be an integer number")
                                ...
                                try:  # To avoid non integer input
                                    ...
                                    while True:
                                        ...
                                        Admin_choice = Admin_choice.upper()
                                        if Admin_choice == "1":
                                            Doctor_DataBase[Doctor_ID][0][0]=input("Enter Doctor's Department : ")
                                            print("/----------------------Doctor's department edited successfully----------------------/")
                                        elif Admin_choice == "2":
                                            Doctor_DataBase[Doctor_ID][0][1]=input("Enter Doctor's Name : ")
                                            print("----------------------Doctor's name edited successfully----------------------")
                                        elif Admin_choice == "3":
                                            Doctor_DataBase[Doctor_ID][0][2]=input("Enter Doctor's Address : ")
                        ...
                        if Admin_choice == "1":  # Admin mode ; Appointment Management ; Book an appointment
                            try:  # To avoid non integer input
                                ...
                                if Admin_choice == "1":
                                    ...
                                    while patient_ID in Patients_DataBase:  # if Admin entered used ID
                                        patient_ID = int(input("This ID is unavailable, please try another ID : "))
                                    Department = Doctor_DataBase[Doctor_ID][0][0]
                                    DoctorName = Doctor_DataBase[Doctor_ID][0][1]
                                    Name = input("Enter patient name    : ")
                                    Age = input("Enter patient age     : ")
                                    Gender = input("Enter patient gender  : ")
                                    Address = input("Enter patient address : ")
                                ...
                                while Session_Start[:2] == "11" or Session_Start[:2] == "12" :
                                    ...
                                    for i in Doctor_DataBase[Doctor_ID] :
                                        if type(i[0])!=str :
                                ...
                                Doctor_DataBase[Doctor_ID].append(New_Appointment)
                            ...
                            try:  # To avoid non integer input
                                ...
                                try:  # To avoid no return function
                                    ...
                                    Doctor_DataBase[PairKey][AppointmentIndex]=[patient_ID,Session_Start,Session_End]
                            ...
                            try:  # To avoid non integer input
                                ...
                                try:
                                    Appointment_index, PairKey = AppointmentIndexInDoctorsDataBase(patient_ID)
                                    Doctor_DataBase[PairKey].pop(Appointment_index)
                    ...
                    if tries==2:
                        ...
                        break
                Write_Hospital_Excel_Sheet.Write_Patient_DataBase(Patients_DataBase)
                Write_Hospital_Excel_Sheet.Write_Doctors_Database(Doctor_DataBase)
        elif Admin_user_mode == "A":  # User mode
            ...
            while True:
                ...
                if UserOptions == "1":  # User mode ; view hospital's departments
                    ...
                    for i in Doctor_DataBase:
                        print(" " + Doctor_DataBase[i][0][0])
                elif UserOptions == "2":  # User mode ; view hospital's Doctors
                    print("Hospital's doctors :")
                    for i in Doctor_DataBase:
                        print(" " + Doctor_DataBase[i][0][1]+" in "+Doctor_DataBase[i][0][0]+" department, from "+Doctor_DataBase[i][0][2])
                    ...
                    try:  # To avoid non integer input
                        ...
                        while Doctor_ID not in Doctor_DataBase:
                            Doctor_ID = int(input("Incorrect Id, Please enter doctor ID : "))
                        print(Doctor_DataBase[Doctor_ID][0][1]+" has appointments :")
                        for i in Doctor_DataBase[Doctor_ID] :
                            if type(i[0])==str :
                                continue
                            else:
